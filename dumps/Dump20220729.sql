...
BEGIN	
	/*Se eliminan los que dicen DEMO*/
    DELETE from tmp_access_point WHERE name_ap like('%DEMO%');
    
    /*Se eliminan los que no coincide la llave*/
...
BEGIN	
	/*Se eliminan los que dicen DEMO*/
    /*DELETE FROM tmp_rs where name_rs like('%DEMO%');*/
    /*Se eliminan los que no se vinculan con el centro digital*/
    /*DELETE from tmp_rs WHERE name_rs NOT IN(SELECT llave from centro_digital where llave = name_rs );*/
    /*DELETE FROM equipo where TIPO IN('RD-CD');*/
	/*INSERT INTO equipo(id_beneficiario,tipo,mac)
	SELECT 	B.id_beneficiario,'RD-CD',min(A.mac) AS MAC
	FROM tmp_rs A
	INNER JOIN centro_digital B ON A.name_rs = B.llave
...
-- Dump completed on 2022-07-29  8:27:28
