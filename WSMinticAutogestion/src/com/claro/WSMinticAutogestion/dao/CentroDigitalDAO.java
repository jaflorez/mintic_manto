		...
		try {
			if(this.connection.isValid(10)) {
				if(id_beneficiario.length() == 5) {
					preparedStatement = this.connection.prepareStatement("SELECT id_beneficiario,llave,id_mintic,nombre_ct_pob,municipio,departamento,vlan,ip_router,server_data,ap_id from centro_digital where id_beneficiario = ?");
				}
				else if (id_beneficiario.length() == 8) {
					preparedStatement = this.connection.prepareStatement("SELECT id_beneficiario,llave,id_mintic,nombre_ct_pob,municipio,departamento,vlan,ip_router,server_data,ap_id from centro_digital where id_mintic = ?");
				}
